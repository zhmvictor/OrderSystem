...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            usersAjax2(0,5);//初始化页面数据（当前是第1页，每一页显示5条数据）
        });
        function usersAjax2(pageIndex,everyPageDataCount){//初始化数据

            $.ajax({
                type: "POST",
                url: "ToPayListServlet",
                data: {
                    ...
                    if(json.myData!=""){
                        ...
                        for(var i=0;i<json.myData.length;i++){
                            ...
                            listHtml+="<tr>"+
                                ...
                                "<td>"+json.myData[i].waiterId+"</td>"+
                                "<td>"+json.myData[i].orderBeginDate+"</td>"+
                                "<td><a class='btn btn-danger' style='width:350px' onclick='paylist("+json.myData[i].orderId +")'>买单</a></td>"+

                                +"</tr>";
                        }

                        $("#listDivId").html(listHtml);
                        pagingAjax2(json.dataCount,everyPageDataCount,json.pageIndex)
                    }else{
                        var listHtml="";
                        $("#listDivId").html(listHtml);
