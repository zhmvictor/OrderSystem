...
$(document).ready(function () {
	showOrder();
	getTopRecommendDishes();
});

    function showOrder(){
    	$.ajax({
	        type: "POST",
	        url: "GetRTPayOrderServlet",
	        data: {
	        },
	        dataType: "json",
	        //出错的回调方法
	        ...
	        success: function (json) {
	        	...
	        	if(json.Arr!=""){
	        		for(var i=0;i<json.Arr.length;i++){
	        			newLine += "<tr><td>" + json.Arr[i].tableId + "</td><td>" + json.Arr[i].orderBeginDate
						+ "</td><td>" + json.Arr[i].orderEndDate + "</td><td>" + json.Sum[i]
						+ "</td>";
				newLine += "<td>";
				...
				newLine += "class='icon-credit-card icon-large' title='确认结账' onclick='pay("
						+ json.Arr[i].orderId + ")'></i>&nbsp;&nbsp;";

				newLine += "<i style='cursor: pointer; font-size: 14;'";
				newLine += "onmouseover='this.style.color=\"orange\"'";
				newLine += "onmouseout='this.style.color=\"black\"'";
				newLine += "class='icon-sitemap icon-large' onclick='showOrderDetail("
						+ json.Arr[i].orderId +","+ json.Sum[i]+ ")' title='查看订单详情'></i>&nbsp;&nbsp;";
				newLine += "<i style='cursor: pointer; font-size: 14;'";
				newLine += "onmouseover='this.style.color=\"orange\"'";
				newLine += "onmouseout='this.style.color=\"black\"'";
				newLine += "class='icon-remove-sign icon-large' title='订单作废' onclick='cancel("
						+ json.Arr[i].orderId + ")'></i>";
				newLine += "</td></tr>";
				/* alert("onclick='showOrderDetail("
					+ json.Arr[i].orderId +","+ json.Sum[i]+ ")'"); */
	        		}
	    			$("#orderTable").html(newLine); 
	        	}
	        	else{
	        		alert("null");
	        	}
    ...
    function showOrderDetail(id,sumPrice){
    	$.ajax({
	        type: "POST",
	        url: "GetOrderDetailServlet",
	        data: {
	        	id:id,
	        	sumPrice:sumPrice
	        },
	        ...
	        success: function (json) {
	        	if(json.Arr!=""){
	        		...
	        		tableId.innerHTML = json.order.tableId;
	        		orderBeginTime.innerHTML = json.order.orderBeginDate;
	        		orderEndTime.innerHTML = json.order.orderEndDate;
	        		sumPrice.innerHTML = json.sumPrice;
	        		userAccount.innerHTML = json.userAccount; 

	        		 for ( var i = 0; i < json.Arr.length; i++) {
	        			var newLine = "<tr>";
	        			newLine += "<td>" + json.Dishes[i].dishesName + "</td>";
	        			newLine += "<td>" + json.Dishes[i].dishesPrice + "</td>";
	        			newLine += "<td>" + json.Arr[i].num + "</td>";
	        			newLine += "</tr>";
	...
	function getTopRecommendDishes() {

		var table = document.getElementById("dishesbord");
		table.innerHTML = "";
		$.ajax({
	        type: "POST",
	        url: "GetTop4RecommendDishesServlet",
	        data: {
	        },
	        dataType: "json",
	        //出错的回调方法
	        ...
	        success: function (json) {
	        	if(json.Arr!=""){
	        		for(var i=0;i<json.Arr.length;i++){
	        			var subDiscript = json.Arr[i].dishesDiscript.length > 8 ? json.Arr[i].dishesDiscript
	        					.substring(0, 7)
	        					+ "..." : json.Arr[i].dishesDiscript;
	        			var newLine = "<div class='col-xs-6 col-sm-3 placeholder'>"+
                            " <a  onclick='detail(\""
                            + json.Arr[i].dishesName
                            + "\",\""
                            + json.Arr[i].dishesDiscript
                            + "\",\""
                            + json.Arr[i].dishesTxt
                            + "\","
                            + json.Arr[i].dishesPrice
                            + ","
                            + json.Arr[i].recommend
                            + ",\""
                            + json.Arr[i].dishesImg + "\")'>";
	        			...
	        			newLine += "style='border-radius:20px;width:95%;height:200px' src='img/dishes/"
	        					+ json.Arr[i].dishesImg + "'></a>";
	        			newLine += "<h4>" + json.Arr[i].dishesName + "</h4>";
		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				<ul class="nav nav-sidebar">
					...
					<li class="active"><a href="pages/admin/main.jsp"><i
				...
				<div class="panel panel-danger">
					...
					<div class="panel-body" style="padding-bottom: 0px">
						...
						<ul class="pager" style="margin-right: 20px">
							...
							<li class="next"><a href="ToDishesAdminServlet">更多特色菜品 <span
