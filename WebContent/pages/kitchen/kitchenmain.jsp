...
<head>
	...
	<script type="text/javascript">
        ...
        function usersAjax(pageIndex,everyPageDataCount){//初始化数据
            $.ajax({
                ...
                success: function (json) {
                    if(json.tableId!=""){
                        var listHtml = "";
                        for(var i=0;i<json.tableId.length;i++){
                            listHtml+= "<tr><td>"+
                                json.tableId[i]+"</td><td>"
                                +json.dishesName[i]+"</td><td>"
                                +json.num[i]+"</td>";
                            listHtml+="<td><a href='javascript:void(0)' onclick='deal(this,\""
                                + json.tableId[i]
                                + "\",\""
                                + json.dishesName[i]
                                + "\",\""+json.Arr[i].odId+"\")' class='";   //+"btn btn-success'  style='width:350px'>开始烹饪</a>"
                            if(json.Arr[i].dishstatic == 0){
                                listHtml+="btn btn-success'  style='width:350px'>开始烹饪</a>";
                            }
                            else{
                                listHtml+="btn btn-danger'  style='width:350px'>正在烹制</a>";
                            }
                            listHtml += "</td></tr>";
                        }
        ...
        $(document).ready(function () {
            usersAjax(0,5);//初始化页面数据（当前是第1页，每一页显示5条数据）
        });



        function deal(obj, tableId, dishesName,odId) {
...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			...
			<ol class="breadcrumb">
				<li><a href="pages/kitchen/kitchenmain.jsp">首页</a></li>
				<li>厨房</li>
